...
```text
Recommendation: <patch | validate_exposure | hunt | monitor | review_controls | ignore>
Why now: <risk justification tied to evidence and environment>
Confidence: <score/label plus caveats>
...
```text
entity_resolution -> evidence(query mode) -> relationships -> ioc_assessment(query mode)
```
...
```text
prioritize -> evidence(top signal) -> relationships(top signal) -> assessment(top material item)
```
